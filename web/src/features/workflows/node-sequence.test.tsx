...
describe("a node with no evidence says which nothing that is (M15 Task 9)", () => {
  it("says a node that ran produced none of it, rather than showing nothing at all", () => {
    // The defect this closes: `EvidenceDisclosure` returned null on an empty evidence set, so a
    // node that ran and recorded nothing was indistinguishable from one the reader had simply
    // not expanded. Those are different facts, and this console's whole argument is that it does
    // not render one nothing as another.
    render(<NodeSequence nodes={[node({ name: "static_verify", status: "done", standing: "ran" })]} />)

    expect(screen.getByText(/ran and recorded none/i)).toBeTruthy()
  })

  it("does not say it about a node that has evidence", () => {
    render(
      <NodeSequence
        nodes={[node({ name: "static_verify", status: "done", standing: "ran", evidence: { diagnostics: [] } })]}
      />,
    )

    expect(screen.queryByText(/ran and recorded none/i)).toBeNull()
  })

  it.each([
    // Matched on the claim only the sentence makes -- the label beside the name already says
    // "owes this node a visit", so asserting that would pass without the sentence rendering.
    ["due", /does not guess between them/i],
    ["not_reached_yet", /still in flight/i],
    ["never_reached", /ended before the graph got here/i],
  ] as const)("gives %s its full sentence rather than the label alone", (standing, sentence) => {
    // The label beside the name is three words. A reader deciding whether a run is stuck needs
    // the sentence that says a parked run and a dead run look identical from a checkpoint —
    // which is the honesty `node-standing.ts` was written to carry and which nothing rendered.
    render(<NodeSequence nodes={[node({ standing })]} />)

    expect(screen.getByText(sentence)).toBeTruthy()
  })

  it("still shows the evidence a node did record", () => {
    render(
      <NodeSequence
        nodes={[node({ name: "static_verify", status: "done", standing: "ran", evidence: { diagnostics: [] } })]}
      />,
