  ...
  const coverageState: CoverageState = coverage.isError
    ...
    : coverage.data?.repo_id === repoId
      ? {
          ...
          // `?? []` is a hotfix from the UI session (2026-08-19 evening): the running API does
          // not serve `by_service` yet, so an unguarded read crashed the whole page for the
          // owner mid-demo-prep. Whoever lands the by_service payload should replace this
          // fallback with the honest three-state the field deserves.
