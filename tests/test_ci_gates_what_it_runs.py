...
# The publication check that used to live here **required the readback to be in a different
# step than the write**, and that is precisely what made it unsatisfiable: GITHUB_STEP_SUMMARY
# is a different file for every step, so the checking step always read an empty file and the
# job failed on every push. The property it was reaching for is right and is now pinned
# correctly in `tests/test_ci_publishes_the_gate_report.py`, against the one-step arrangement.
# Not restated here: one fact written in two files is one fact that will disagree with itself.
