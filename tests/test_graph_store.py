...
# -- Finding dismissal: decision 45, and the grain is the whole of it ------------------------
#
...
# latest; a boolean column on `finding` could not show that somebody changed their mind, and
# `false positive` is the only honest source of detector accuracy, which needs the history.


def _dismissable(store) -> str:
    site_id = store.upsert_call_site(_site())
    return store.insert_finding(
        Finding(
...
def test_a_dismissal_is_an_event_rather_than_a_property_of_the_finding(store):
    finding_id = _dismissable(store)
    store.dismiss_finding(finding_id, reason="false-positive", actor="sebastian", at=SEEN)

    current = store.current_dismissal(finding_id)

    assert current is not None
    assert current["reason"] == "false-positive"
    assert current["actor"] == "sebastian"


def test_a_finding_nobody_dismissed_has_no_dismissal(store):
    finding_id = _dismissable(store)

    # Absence rather than a row saying "not dismissed" -- the table records what happened, and
    # nothing happening leaves nothing behind.
    assert store.current_dismissal(finding_id) is None


def test_reinstating_writes_a_second_row_and_the_latest_one_wins(store):
    finding_id = _dismissable(store)
    store.dismiss_finding(finding_id, reason="wont-fix", actor="sebastian", at=SEEN)
    later = datetime(2026, 7, 21, 12, tzinfo=timezone.utc)
    store.reinstate_finding(finding_id, actor="sebastian", at=later)

    # The dismissal is not deleted. Somebody changed their mind, and that is the fact the history
    # exists to hold -- a boolean flipped back would leave no trace it was ever set.
    assert store.current_dismissal(finding_id) is None
    assert len(store.dismissal_history(finding_id)) == 2


def test_dismissing_again_after_reinstating_is_dismissed_again(store):
    finding_id = _dismissable(store)
    store.dismiss_finding(finding_id, reason="intentional", actor="a", at=SEEN)
    store.reinstate_finding(finding_id, actor="b", at=datetime(2026, 7, 21, 12, tzinfo=timezone.utc))
    store.dismiss_finding(
        finding_id, reason="not-used-here", actor="c", at=datetime(2026, 7, 22, 12, tzinfo=timezone.utc)
    )

    current = store.current_dismissal(finding_id)

    assert current is not None
    assert current["reason"] == "not-used-here"
    assert len(store.dismissal_history(finding_id)) == 3


def test_the_same_event_written_twice_converges(store):
    """Every table gets a natural key and an explicit conflict clause -- `CLAUDE.md`, and `efcc19d`
    was this bug. One person dismissing one finding at one instant is one event however many times
    the write is replayed."""
    finding_id = _dismissable(store)
    store.dismiss_finding(finding_id, reason="false-positive", actor="sebastian", at=SEEN)
    store.dismiss_finding(finding_id, reason="false-positive", actor="sebastian", at=SEEN)

    assert len(store.dismissal_history(finding_id)) == 1


def test_a_reason_outside_the_closed_vocabulary_is_refused(store):
    finding_id = _dismissable(store)

    # Closed for the reason `abandon_reason_code` is: a promise to learn from dismissals needs a
    # schema that can answer the question, and free text cannot be aggregated.
    with pytest.raises(ValueError):
        store.dismiss_finding(finding_id, reason="because I said so", actor="a", at=SEEN)


def test_dismissed_findings_are_listed_so_a_screen_can_filter_rather_than_delete(store):
    kept = _dismissable(store)
    other_site = store.upsert_call_site(_site(path="src/other.ts", line=9, content_hash="hash-9"))
    dismissed = store.insert_finding(
        Finding(
            detector="vendor-change",
            claim="operation-removed",
            call_site_id=other_site,
            severity="breaking",
            rationale="the operation this call names is gone",
            binding_rung="static",
        )
    )
    store.dismiss_finding(dismissed, reason="intentional", actor="a", at=SEEN)

    # Filtered out by default, never deleted: the row stays listed so a reader can ask what was
    # set aside and why.
    assert store.dismissed_finding_ids() == {dismissed}
    assert kept not in store.dismissed_finding_ids()
