...
> Status: **supported** -- a registered `generated` adapter serves `perplexity`.
...
uv run sync run --vendor perplexity \
    --from-version <pinned> --to-version <target> --repo <your-remote>
```

The console then shows every call site bound to Perplexity operations, each finding with the provenance rung it arrived at.

## What Sync watches

Sync reads the manifest this vendor's SDK generator commits to `perplexityai/perplexity-py`, fetches the specification the manifest names when its hash moves, and diffs pinned versions with oasdiff. No agreement with the vendor is required and none can be withdrawn: the manifest is what the generator writes for its own reasons.
...
Official documentation: [https://docs.perplexity.ai](https://docs.perplexity.ai)
