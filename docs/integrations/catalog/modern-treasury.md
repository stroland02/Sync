...
> Status: **supported** -- a registered `generated` adapter serves `modern-treasury`.
...
uv run sync run --vendor modern-treasury \
    --from-version <pinned> --to-version <target> --repo <your-remote>
```

The console then shows every call site bound to Modern Treasury operations, each finding with the provenance rung it arrived at.

## What Sync watches

Sync reads the manifest this vendor's SDK generator commits to `modern-treasury/modern-treasury-python`, fetches the specification the manifest names when its hash moves, and diffs pinned versions with oasdiff. No agreement with the vendor is required and none can be withdrawn: the manifest is what the generator writes for its own reasons.
...
- **python**: `modern-treasury`
Official documentation: [https://docs.moderntreasury.com](https://docs.moderntreasury.com)
