# Integration catalog

Generated from the vendor registry by `scripts/build_integration_docs.py` -- the same call the command line resolves vendors with, so this table cannot claim an integration the product does not serve. *Supported* means a registered adapter watches the vendor today; *recognized* means Sync can name the dependency and says so, and watching it is described on its page.

Missing a vendor entirely? [Writing a vendor adapter](../../writing-a-vendor-adapter.md) is the path, and an adapter depends on `sync.core` alone.

## Supported

| Vendor | Adapter kind | Categories |
|---|---|---|
| [Anthropic](anthropic.md) | generated | ai |
| [Browserbase](browserbase.md) | generated | dev-tools |
| [Cloudflare](cloudflare.md) | generated | infrastructure |
| [Finch](finch.md) | generated | hr |
| [Groq](groq.md) | generated | ai |
| [Increase](increase.md) | generated | banking |
| [Lithic](lithic.md) | generated | payment |
| [Mistral AI](mistral.md) | generated | ai |
| [Modern Treasury](modern-treasury.md) | generated | banking |
| [OpenAI](openai.md) | generated | ai |
| [Openlayer](openlayer.md) | generated | ai |
| [Orb](orb.md) | generated | invoicing |
| [Stripe](stripe.md) | coded | payment |
| [Twilio](twilio.md) | coded | communication |
| [Vercel](vercel.md) | generated | infrastructure |

## Recognized

| Vendor | Categories |
|---|---|
| [Airtable](airtable.md) | productivity |
| [Algolia](algolia.md) | dev-tools, search |
| [Amazon Selling Partner API](amazon-selling-partner.md) | dev-tools, e-commerce |
| [Apify](apify.md) | dev-tools, infrastructure |
| [Atlassian](atlassian.md) | dev-tools, productivity |
| [AWS](aws.md) | dev-tools, infrastructure |
| [Axiom](axiom.md) | dev-tools, analytics |
| [Azure DevOps](azure-devops.md) | dev-tools |
| [Bitbucket](bitbucket.md) | dev-tools |
| [Builder.io](builder-io.md) | dev-tools, cms |
| [Canva](canva.md) | dev-tools, productivity |
| [Clerk](clerk.md) | dev-tools, iam |
| [ClickHouse](clickhouse.md) | dev-tools, infrastructure |
| [Codegen](codegen.md) | dev-tools, ai |
| [Contentful](contentful.md) | dev-tools, cms |
| [CrowdStrike](crowdstrike.md) | dev-tools |
| [Cursor](cursor.md) | dev-tools, ai |
| [Datadog](datadog.md) | dev-tools, analytics |
| [DigitalOcean](digitalocean.md) | dev-tools, infrastructure |
| [Drata](drata.md) | dev-tools |
| [Dub](dub.md) | dev-tools, marketing |
| [Dynatrace](dynatrace.md) | dev-tools, analytics |
| [ElevenLabs](elevenlabs.md) | dev-tools, ai |
| [fal](fal-ai.md) | dev-tools, ai |
| [Firebase](firebase.md) | dev-tools, infrastructure |
| [Ghost](ghost.md) | dev-tools, cms |
| [GitHub](github.md) | dev-tools |
| [GitLab](gitlab.md) | dev-tools |
| [Google Gemini](google-gemini.md) | dev-tools, ai |
| [Google Play](google-play.md) | dev-tools, e-commerce |
| [Grafana](grafana.md) | dev-tools, analytics |
| [HubSpot](hubspot.md) | crm |
| [Intercom](intercom.md) | support |
| [Jira](jira.md) | dev-tools, ticketing |
| [Juniper Mist](juniper-mist.md) | dev-tools, infrastructure |
| [LaunchDarkly](launchdarkly.md) | dev-tools |
| [Linear](linear.md) | ticketing |
| [Microsoft Intune](microsoft-intune.md) | dev-tools, infrastructure |
| [n8n](n8n.md) | dev-tools, productivity |
| [Neon](neon.md) | dev-tools, infrastructure |
| [Netlify](netlify.md) | dev-tools, infrastructure |
| [Notion](notion.md) | productivity |
| [Okta](okta.md) | dev-tools, iam |
| [OneLogin](onelogin.md) | dev-tools, iam |
| [Oracle Cloud Identity](oracle-cloud-identity.md) | dev-tools, iam |
| [PagerDuty](pagerduty.md) | dev-tools |
| [Perplexity](perplexity.md) | dev-tools, ai |
| [Phrase](phrase.md) | dev-tools |
| [PingOne](pingone.md) | dev-tools, iam |
| [Pipedream](pipedream.md) | dev-tools, infrastructure |
| [PostHog](posthog.md) | dev-tools, analytics |
| [Ragie](ragie.md) | dev-tools, ai |
| [Recall.ai](recall-ai.md) | dev-tools, communication |
| [Render](render.md) | dev-tools, infrastructure |
| [Replicate](replicate.md) | dev-tools, ai |
| [Resend](resend.md) | dev-tools, communication |
| [Retell AI](retell-ai.md) | dev-tools, ai |
| [Rootly](rootly.md) | dev-tools, infrastructure |
| [Salesforce](salesforce.md) | crm |
| [SendGrid](sendgrid.md) | communication |
| [Sentry](sentry.md) | dev-tools, analytics |
| [Shopify](shopify.md) | e-commerce |
| [Shortcut](shortcut.md) | dev-tools, productivity |
| [Slack](slack.md) | communication |
| [Snowflake](snowflake.md) | dev-tools, analytics |
| [Statsig](statsig.md) | dev-tools, analytics |
| [Supabase](supabase.md) | dev-tools, infrastructure |
| [Trigger.dev](trigger-dev.md) | dev-tools, infrastructure |
| [Webflow](webflow.md) | dev-tools, cms |
| [WooCommerce](woocommerce.md) | dev-tools, e-commerce |
| [WordPress](wordpress.md) | dev-tools, cms |
| [xAI](xai.md) | dev-tools, ai |
| [Zapier](zapier.md) | dev-tools, productivity |
