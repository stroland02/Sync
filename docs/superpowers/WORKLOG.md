...
| M14-W429 | **Decision 41's durable half: `index_run`, and the pass that writes it.** A toast may announce *Index finished, 1,204 call sites* but must never be the only place that fact lives. Nothing recorded a pass completing: `call_site.indexed_at` says rows exist, and rows exist just as loudly after a run that died halfway -- a count read from them renders as a completed index of a smaller codebase, which is a false claim reached by arithmetic rather than asserted by anyone. **Grain declared before the columns: one row per index PASS, per repository**, identity (repo_id, started_at), conflict clause on both writes so re-running INDEX converges. `finished_at IS NULL` is a pass that did not finish and `call_sites` stays NULL with it, because a partial count is not a smaller count, it is not a count. **Opened before the pass rather than written after it**, which is the only ordering that can leave evidence of a run that died. Wired into `cli.py`'s real index pass and held by a scan test, so it is not another table nothing writes. Four test doubles gained the two writes rather than the production path gaining a `hasattr` guard -- a guard there would silently stop recording if a name ever drifted. This also gives decision 61's empty table the *what was checked* sentence it needs | landed | `src/sync/graph/schema.sql`, `src/sync/graph/store.py`, `src/sync/cli.py` |
...
| CI-W431 | `finding.opened` on the bus -- the second publisher decision 76 names, and the one decision 87 needs before a new-findings banner can exist | claimed | `src/sync/graph/store.py` |
