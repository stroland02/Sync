...
| CI-W635 | **Call sites becomes a locked explorer, the detail becomes a drawer, and the console goes full width.** Step three, with two owner corrections landed the same session. The screen stamps `data-screen="locked"` -- the first to do so -- which closed the positive half of the frame's `:has()` behaviour: measured live, `main` flips to `overflow-hidden`, the document does not scroll, and only the panes carry scrollbars. **Two structural mistakes of mine, both caught by the owner's captures**: I nested a grid inside `DetailLayout`, which already owns the table/inspector split, and the table was crushed until file paths wrapped one word per line; and the docked column was the wrong shape entirely -- the detail is a right-hand **drawer over** the page now, so a reader working a table keeps the table at full width. `max-w-[1400px]` is deleted on every route: 1920 is the width this console is operated at, and the cap left 520px of dead page beside a table that wanted it | landed | `call-sites-page.tsx`, `detail-layout.tsx`, `filter-rail.tsx`, `app-frame.tsx` |
