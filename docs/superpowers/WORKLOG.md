...
| M10-W240 | B151: `main` was 60 tests red because `12e416a` dropped `report_reason`, `static_attempts` and `ci_attempts` from `RunState`'s `TypedDict` while `nodes.py` kept reading and writing all three. `StateGraph(RunState)` builds one channel per declared key, so a node returning an undeclared key is a write nothing reads, silently -- the two counters never actually incremented, so the retry budget never tripped (`GraphRecursionError`), and `report_reason` came back as a bare `KeyError`. Fix restores all three where their own surviving comments already said they belonged. Also fixes `test_remediation_graph.py`'s pinned topology (`park`/`external_cause`, added by the same commit, never pinned) and baselines two finished-but-uncalled dead links (`ensure_image_built`, `dispatch_webhook_event`) with the reason rather than force-wiring a caller. **Committed as `M0-W241` before this row existed** -- that number collides with the coordinator's own real `M0-W241` (the Lane-A/Lane-E duplication postmortem); this row is the correction, the commit hash (`dba90a5`) is unchanged since it is still local and unpushed | landed | `src/sync/remediate/state.py`, `tests/test_remediation_graph.py`, `scripts/dead_links_baseline.txt` |
