...
| CI-W413 | **Red main, and I caused it**: my merge loop staged six conflict-marked files with `git add -A` after resolving only WORKLOG -- 157 tsc errors. Repaired, and the loop now refuses to stage while any marker survives | claimed | `web/src/features/` |
