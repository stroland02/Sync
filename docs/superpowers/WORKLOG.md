...
| M14-W436 | **P0: every finding and vendor link in the console pointed at a route that no longer exists.** `M14-W386` scoped all ten routes under `/repositories/:repoId/`; twelve non-test link sites still built the old unscoped `/findings/{id}` and `/vendors/{id}` paths, and `App.tsx` has a `path="*"` catch-all with no legacy redirect -- so each one rendered *No screen at this address*. The build was green throughout, and so were 771 console tests: a `<Link to>` is a string, and nothing typechecks a string against a route table. Ten sites had `repoId` as a prop or a route param and are mechanical. Two did not and are not in this unit -- `runs-table` and `proposed-patch` link from a run, and `RunRow` carries no repository because `migration_outcome` stores none, so the destination is genuinely underdetermined. **The guard is the deliverable**, not the twelve edits: a link whose first path segment is not a declared route root now fails a test rather than a reader's click | claimed | `tests/test_console_links_resolve.py`, ten `web/src` link sites |
| CI-W437 | **Decision 45's write path: the three dismissal methods finally have a caller.** `record_dismissal`, `dismissal_state` and `dismissal_history_count` were built, tested and reached from nowhere -- the pattern the standing rule names, that a unit is done when something in `src/` calls it. `GET`/`POST /api/findings/{finding_id}/dismissal`. **The route refuses a dismissal it cannot attribute, and that is the substance rather than validation hygiene:** `actor` exists so a reader can ask *who decided*, `false_positive` is the only honest source of detector accuracy, and a console behind one shared password cannot infer who is typing. So the caller says who or the write is refused -- filling the column with a constant would satisfy the schema and destroy the only question it exists to answer. Proven by defaulting the actor to `console` and watching the guard go red. **The closed vocabulary is not restated in the transport**: `record_dismissal` owns it and raises naming it, the route turns that into a 400 carrying the store's own words, and a second copy in the API would be the one that drifts first. Restoring writes a second row rather than erasing the first, so the console can show that somebody changed their mind. **Its commit says `CI-W436`, which another lane won at push while the quiet window made claiming impossible** -- the window suspends the very mechanism adopted after `CI-W426` to prevent exactly this | landed | `src/sync/api/`, `tests/test_api_dismissal.py` |
