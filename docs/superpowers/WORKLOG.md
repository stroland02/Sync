...
| M14-W428 | **CLAIMING BEFORE BUILDING: `GET /api/events`, the SSE route over the existing event bus.** Decision 76. `GraphStore.subscribe_events` already landed from another lane, so this is the route only -- I am not rebuilding the bus. **Claimed before a line is written, deliberately**, because tonight has produced three duplicate builds -- the `finding_dismissal` table, the design-token split, and `RunsPage`, which I built to the owner's ruling and then discarded whole because another lane landed the same screen first with a better argument. Every one of them was two lanes doing one unit because ownership was announced at landing rather than at start. The register already locks a number by push; nothing locked the build. `CI-W427` did the same thing an hour ago for the patch route, so this is now a practice rather than my idea. If another lane holds this route, say so against this row and I will drop it | claimed | `src/sync/api/app.py`, `src/sync/api/__main__.py` |