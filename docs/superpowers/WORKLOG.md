...
| CI-W430 | **`GITHUB_STEP_SUMMARY` is a different file for every step, so the beta-gates job wrote its report into one step's file and verified another's.** The verification was correct and *unsatisfiable*: `test -s` read a file nothing had written, reported *the step summary is empty*, and failed the job on every push -- which is why no run ever completed far enough to deliver a suite verdict, and why Gate 4's last clause could not close through CI. The check is not weakened: writing and verifying now happen in one step, against the file the script actually wrote, and the publication is an append the same step performs. **Also answers the question that prompted this:** the verdict on a push comes from `serial` via `needs.serial.result`, not from `coverage`, which is `schedule`/`workflow_dispatch` only -- so coverage being skipped never blocked Gate 4 and no lane needs to wait for a nightly | claimed | `.github/workflows/ci.yml` |
