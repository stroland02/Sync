...
| M14-W411 | RESERVED before the work: coordinator review of M14-W410 found `DockerSdkRunner` had zero non-test callers (`grep -rn DockerSdkRunner src/` returned only the class definition) and two of the three baselined symbols still had real entries -- `copy_between_containers` and `ensure_image_built` were never actually reached, only `ephemeral_container` was. Wiring `AgentRemediator.__init__` to select `DockerSdkRunner` behind an env flag (`SYNC_PATCH_SANDBOX`, off everywhere today); wiring `ensure_image_built` as the real image-tag source; deciding `copy_between_containers`, which the risky/safe container pair it was built for turned out to be superseded by `DockerSdkRunner`'s own proxy+isolated-network design -- deleted rather than kept unreached, with its test rewritten to keep the one property it proved that is still load-bearing. Also fixes a bug found while re-checking Gate 4's own `_sandbox_wired`: a substring search over the whole baseline file misread a retirement comment naming a symbol as a live entry for it | reserved | `src/sync/remediate/agent_patch.py`, `src/sync/runner/docker_sdk.py`, `src/sync/remediate/sandbox.py`, `scripts/beta_gates.py` |
...
| CI-W419 | Decision 76 as a real bus rather than polling behind a push facade -- INDEX and DETECT publish on write, and `GET /api/repositories/{id}/events` forwards. Owner-selected: real LISTEN/NOTIFY, per-repository path, freeze-and-name on drop | claimed | `src/sync/graph/store.py`, `src/sync/api/app.py` |
...
| M14-W421 | **Two owner rulings, and the Python gate is green again.** (1) The focus-ring and geometry guards now skip `components/ui/` as well as `vendor/supabase/`: `ring-ring/50` and `transition-all` are shadcn's own defaults, arriving with the catalog, and a guard failing on somebody else's defaults reports a decision nobody here made. The keyframe guard already excluded that directory on exactly this argument, so the two prefixes are now one `_VENDORED_PREFIXES` tuple with one `_is_vendored` helper rather than a third spelling -- a guard quietly covering less than its neighbour is the shape that disagreement would take. Three guards red on `main` for hours, now green, and no guard was widened for `features/`. (2) `finding_dismissal` gained the natural key and conflict clause `CLAUDE.md` requires of every table: `UNIQUE (finding_id, actor, created_at)` with `ON CONFLICT ... DO UPDATE`. `record_dismissal` takes an optional `at`, so a replayed write converges instead of recording one decision twice, while two genuine clicks land at different instants and stay two rows -- the key must not collapse a real change of mind, which is the whole reason this is rows and not a column. The four dismissal reads are baselined with the caller that retires them | landed | `tests/test_console_design_tokens.py`, `src/sync/graph/schema.sql`, `src/sync/graph/store.py`, `scripts/dead_links_baseline.txt` |
