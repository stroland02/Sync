...
| M14-W410 | RESERVED before the work: B97, the last Gate 4 clause -- `ephemeral_container`, `copy_between_containers` and `ensure_image_built` are baselined as reached from nowhere, so the sandbox image is built and pre-warmed and nothing runs inside it. Wiring a `DockerSdkRunner` that drives the patch agent inside the sandbox: a two-container split (proxy holds the credential, sandbox never sees it), a real in-container driver, host<->container copy helpers on `sandbox.py`, and their tests. Not wired into `AgentRemediator`'s default and no live model call made -- that needs separate spend authorization | reserved | `docker/patch-sandbox/`, `src/sync/runner/docker_sdk.py`, `src/sync/remediate/sandbox.py` |
