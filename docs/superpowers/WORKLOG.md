...
| CI-W409 | **Red `main`: five design-token guards failing, three of them mine and now green.** Raw spacing across `features/settings`, `features/vendors` and `features/dashboards` -- `gap-1`/`gap-2`/`p-4` spelling values `--spacing-field`/`--spacing-row`/`--spacing-section` already name. **A 10px monogram under DESIGN.md's 12px floor**, which is a floor rather than the small end of a range: a label out of room takes fewer characters, and `monogramFor` already caps at two, so it renders at `--text-meta`. **Colour literals in a chart test** -- and the fix is that the fixture never needed colours: what it proves is that the builder *reads a token through*, not what the token is, so sentinels say it better and stop inventing colours outside `index.css`. Two geometry transitions of mine went too: `transition-all` on policy buttons that only change colour, and **a hover lift on a vendor card removed rather than retimed, because there is no duration at which a meaningless movement becomes meaningful**. **The remaining two guards are entirely `components/ui/**`** -- the vendored shadcn batch -- and are reported rather than taken: the focus-ring fix is mechanical, but `switch.tsx`'s thumb and `progress.tsx`'s width are real affordances and snapping them is a motion ruling, not a lint fix | landed | `web/src/features/` |
| M14-W411 | **`main` carried committed conflict markers in six files and the whole console failed to parse.** `npm run build` died with `PARSE_ERROR` at `settings-page.tsx:105` on a literal `<<<<<<< HEAD`; nothing built, so no lane could gate. Introduced by `6a8be1c7`. **The mechanism is the part worth keeping:** once markers are *committed* they are ordinary file content, so `git status` shows no `UU`, `git merge` reports success, and a gate step checking for `^UU` passes clean -- mine did. Only running the build, or grepping `^<<<<<<<`, catches it, and that grep is now in my gate. Every conflict was the same pair -- `M14-W408`'s tokens against `CI-W409` fixing the identical violations -- so each was resolved by taking whichever side carries no raw 12px value, which is the guard-compliant side in all fourteen. Entered `settings/**` against the ownership split deliberately and said so: main was unbuildable, the fix was mechanical, and holding five lanes on a tree that does not compile costs more than the boundary protects | landed | `web/src/features/settings/`, `web/src/features/vendors/vendor-sources-card.tsx` |
