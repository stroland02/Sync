...
| M7-W225 | Vitest coverage for `NodeEvidence` component across scalars, flags, language-tagged blocks, and unnamed properties; dev server verified live on port 5173 with proxy to 8787 | landed | `web/src/features/workflows/evidence.test.tsx`, `docs/superpowers/WORKLOG.md` |
| M13-W226 | M13 Phase 1: Reasoning & Strategy disclosures in Solution Workflow `NodeEvidence` component, providing structured deductions across AST transforms, compiler checks, sandbox safety, and CI polling inspired by DeepSeek Harness | landed | `web/src/features/workflows/evidence.tsx`, `web/src/features/workflows/evidence.test.tsx`, `docs/superpowers/WORKLOG.md` |
