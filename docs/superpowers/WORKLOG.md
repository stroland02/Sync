...
| CI-W544 | **Demo-v1 gains product groupings, and two of the other lane in-flight crashes got guarded hotfixes.** seed_services writes the operation-to-product mapping an adapter would eventually derive (six products across three vendors) so the Services page original table and bars fill; the taxonomy panel stays above as the breadth answer. In the primary checkout, two unguarded by_service reads (Services page, Vendors ServicesCalled) crashed whole pages while the running API predated the payload -- both now carry a missing-is-its-own-nothing guard commented for whoever lands the field. The stale API on 8787 was the dev-loop trap made real: a zombie wrapper owned the socket and the child served old Python; restarted from the primary tree with SYNC_API_RELOAD=true | landed | scripts/seed_demo_v1.py, plus two guarded hotfixes in the primary tree |
| CI-W543 | **Demo V1: one codebase lights every surface, and the flows wear their outcomes' colours.** Owner directive -- wipe the test workspaces, build one demo codebase showing everything. `github.com/stroland02/demo-v1` is a real repository (furever base + enrichments so every detector fires: twilio calls, a payout loop, anthropic/openai model literals, a python reporting service), pushed public so credentialed clones survive the credential-helper breakage. `scripts/seed_demo_v1.py` is the one-command rebuild: wipe both stores, `sync index` (35 sites, all with captured snippets), a REAL `sync run` (stripe v2320-v2330 diff; 10 findings; one genuine remediation attempt that honestly abandoned on this machine's missing yarn), then the layers that need traffic or a model key nobody has yet: observed telemetry past the status-rate floor (6 observed-rung findings), shapes, error windows, tickets in every lifecycle state on real finding ids, checkpointer runs in `sync.cli`'s own checkpoint shape (opened-with-PR / live / reported), corpus rows, watch subscriptions. Synthetic values, real machinery -- every write goes through the store's own models. **SankeyFlow gains tones and hover**: a node that IS a state wears its outcome's reserved ink so the flows agree with the tags (owner ask, with the drawn reference), hover titles say the whole sentence, structural nodes keep the volume ramp. **The Services page opens on the integration-surface taxonomy**: twelve API classes (REST-SDK, raw OpenAPI, GraphQL, gRPC, webhooks, streaming, queues, SOAP, OAuth, BaaS, LLM, MCP), each honestly tagged watched-today or planned -- a capability catalogue, never graph claims | landed | scripts/seed_demo_v1.py, web/src/components/charts/sankey-flow.tsx, features/vendors/api-service-taxonomy.ts, api-taxonomy-panel.tsx, repository-services-page.tsx, workflows/remediation-flow.tsx, tickets/ticket-funnel.tsx |
| CI-W539 | **The ticket: findings become remediation requests, and the console gets its second write.** Owner rulings of 2026-08-19 -- the Findings page is the manual lane, the watch loop the automatic one, and the platform's differentiator is that the loop runs unattended. `remediation_ticket` (grain: one row per request; a retried finding is a new ticket) with the partial unique index as the idempotence rule -- at most one open ticket per finding, so a double-click and the watch tick racing an operator converge on one row that keeps the lane that asked first, proven red. `claim_next_ticket` moves a row out of requested atomically (FOR UPDATE SKIP LOCKED) so two runners cannot execute one request. **POST /api/findings/{id}/ticket is the write the console calls**: it records the ask and returns -- no route clones, patches, or invokes the graph; `sync tickets` (next slice) is the executor, so a dead runner reads as a ticket parked at requested, never an HTTP timeout pretending to be progress. The read-only ruling is amended in place in `app.py`. Console: the finding rail's Create-remediation-ticket action with the four-state row (button / initiated / in progress / done wearing the run's outcome tone), the Detectors page opens on the **Automatic lane** (watch-created tickets, finding to workflow to PR), and Solutions gains the **ticket funnel** -- the owner's drawn left-to-right split into initiated / in progress / complete-by-outcome, on the existing conserving Sankey. Both poll at 15s with the stamped honesty the Telemetry page set | landed | src/sync/graph/schema.sql, store.py, api/app.py, __main__.py, web/src/features/tickets/*, findings/finding-page.tsx, detectors/detectors-page.tsx, workflows/solutions-page.tsx |
| CI-W537 | **Closed vocabularies wear the status ramp: the tag anatomy gains a tone axis.** (Commit `e8fabdcd` says CI-W536 -- claimed concurrently with another session's landing; the register rules and this row is the claim.) Owner re-ruling on the running console, superseding two same-day refusals recorded in `tag.tsx` and `run-outcome.tsx`. No new colour exists: a tone comes only from the four reserved status colours plus neutral, all already argued in DESIGN.md. The discipline granted with the ruling, held by tests proven red: **a mapping ranks the way its vocabulary ranks** (severity: breaking=critical, warning=serious, deprecation=warning, addition=good, info=neutral -- the crossed mapping was planted and watched fail), **an unknown member renders neutral** rather than wearing a confident wrong colour, and **colour never travels alone** -- every toned tag ships the tone glyph `components/status.tsx` reserves, one shape per tone everywhere. Run outcomes tone their headlines through `Status` (opened=good, abandoned=serious, reported=warning, in-flight and rehearsal neutral); the unregistered adapter tier wears warning as the one tier that is a gap; change kinds stay neutral (two hundred members is a vocabulary no ramp can rank -- the severity beside them carries the state); **the provenance rung stays monochrome**, the one vocabulary the re-ruling did not touch. `console-surface.md` amended in place | landed | web/src/components/tag.tsx, status.tsx, features/workflows/run-outcome.tsx, .claude/rules/console-surface.md |
| CI-W535 | **The Telemetry page becomes the Observe stage: live instrument, owner rulings batched.** The three-role catalogue is retired from this page -- the vendor roster lives on Vendors, the delivery surface in Settings, and roles.ts is deleted rather than deprecated (the M5 table in the spec is untouched; the level stopped being its catalogue, not its authority). What replaces it: **a traffic rollup per operation** at StatusRateDetector own grain and denominator rule -- requests are statused spans, unstatused spans leave both sides and are counted where a reader can discount the rate -- **an hourly requests/errors chart** on the existing day-stack builder (errors on the baseline, the bucketing caveat stated: spans carry no time of their own), **an unattributed-traffic panel** (the correlation gap is a live signal, never pooled into a measurement), and a KPI strip leading with requests, the error sentence with its denominator, and observed-over-indexed coverage. **Polling, not a live badge**: the page re-asks every 15s while open and stamps when it last asked -- a pulse would claim a push this transport does not have. Backend: observed_traffic_rollup / observed_traffic_series / indexed_operation_count on the store, payload extended, rung separation proven red | landed | src/sync/graph/store.py, dashboard/graph_views.py, web/src/features/telemetry/*, signals/* |
...
| CI-W536 | **M15 Task 8: a run waiting on a human read as one in flight, and the vocabulary already held the state.** The plan called for a spec amendment to extend the run vocabulary. It did not need one: `make_park` has always written `outcome: "parked"` with a `parked_reason`, and `Outcome` has always carried it. What it never reached was `_FINISHED`, and **every display site asks `outcome in _FINISHED else None`** while the console renders `None` as *in flight* -- so a run that had stopped, that nobody was coming back to on their own, was reported as busy on the one screen that exists to say otherwise. **`parked` was deliberately not added to `_FINISHED`**: a tuple named for finishing, holding a state that did not finish, is a name that misleads the next reader into counting a parked run as an ending -- and the corpus rates are built on that word. A new `DISPOSITIONS = (*_FINISHED, "parked")` answers the question the display sites are actually asking, *does this run have something to report*, and `_FINISHED` keeps meaning terminal. **Two guards caught the console for me rather than the other way round**: `test_the_consoles_run_disposition_matches_the_finished_outcomes` reads the TypeScript from Python and went red the moment the Python widened, and TypeScript's exhaustive switch in `change-units-table.tsx` stopped compiling -- a disposition the payload can write and no screen can describe would have rendered as a blank cell. The in-flight filter moved with the classification, or the rail would have promised live runs and delivered waiting ones | landed | `src/sync/dashboard/queries.py`, `fleet.py`, `web/src/api/types.ts`, `features/fleet/runs-table.tsx`, `change-units-table.tsx` |
| CI-W538 | **The console showed what was broken and nothing else, so a safe API appeared nowhere.** Owner question, 2026-08-19: *why do severity tables not have a safe category, and why do we not show safe APIs?* The first half has an answer that had to be given rather than built around -- **severity is the vendor's published label on a change**, `oasdiff` emits no `safe`, and a finding exists only where a call site binds to a change, so those tables are lists of problems by construction. A `safe` severity would put a judgement about the customer's codebase inside a column that otherwise carries only the vendor's own words, one step from the traffic light this console refuses. The second half was a real gap: a reader could not tell **"we checked this and nothing binds"** from **"we never checked this"**, which is the distinction `web/CLAUDE.md` says matters most, violated by omission on every screen. So `binding_status` is computed per call -- `at_risk`, `clean`, `unchecked` -- and **`clean` is only honest because `intake_attempt` exists**: that table's own grain says it is there to keep *never-asked* apart from *nothing-new*, so a vendor with no **successful** intake has never had its specification read and its calls are unexamined rather than fine. `declined` and `failed` are not evidence -- an adapter that would not answer and one that could not -- and counting any attempt would turn a week of 403s into an all-clear. Proven by making unexamined default to `clean` and watching five guards go red. **At risk is per operation, not per vendor**: a vendor with one broken call and forty working ones is the ordinary case. Dismissal is deliberately not special-cased -- `_open_findings_predicate`, the clause seven reads share, asks only `finding.status = 'open'`, and teaching one query about dismissals would put *clean* beside a table listing the finding. The tag stays **monochrome** though `DESIGN.md` would permit colour: a green `clean` beside a red `at risk` is a traffic light, and the whole point of `unchecked` is that a reader must not skim past it as though it were mild. Two CTEs rather than per-row `EXISTS`, because the facet groups over the whole set | landed | `src/sync/graph/store.py`, `api/app.py`, `web/src/components/tag.tsx`, `features/bindings/call-sites-page.tsx` |
| CI-W541 | **The API surface panel: the one place on this console where good news is a number.** Second pass on the owner's *show safe APIs* question -- `CI-W538` put the status on the Call sites table, this puts the shape of the whole surface on the Overview. **Counted over operations, not call sites**, and the grain is the point: *how much of my API surface is safe* is a question about breadth, and forty call sites to one operation is one thing to know about -- counting sites would let a single heavily-called operation dominate the answer. Proven by dropping the `DISTINCT` and watching the guard read 3 where 1 was right. **Bars, not a donut**, for the reason `web/CLAUDE.md` records at cost: a donut cannot draw a zero, and a healthy codebase's `at_risk: 0` is precisely the figure a reader came to see -- it would ship as a closed ring reading as broken, exactly as the provenance donut did. **The zero rule is split, and the split is the honest part**: the payload never fills a missing member, because an absent key was not measured at nought; the panel fills it, but only after establishing that a surface exists at all, at which point every operation has one of the three statuses and a member the grouping did not return really is a measured zero. A repository with no call sites gets a sentence instead of three noughts, because three noughts would claim its operations had been examined and found clean. **No percentage, no ratio, no health figure** -- a scalar averaging *we could not check* with *we checked and it passed* collapses the distinction the panel exists to draw, and the guard greps the rendered text for one. Noted in passing and deliberately not touched: `IndexCoverageResponse.by_service` is declared in `types.ts`, sent by no payload and read by no screen -- another session's work in flight | landed | `src/sync/graph/store.py`, `dashboard/graph_views.py`, `web/src/features/repositories/api-surface-panel.tsx`, `codebase-page.tsx` |
| CI-W542 | **A test file hardcoded the development database and silently emptied the console the owner was testing against.** `conftest.pytest_configure` gives a run with `SYNC_DSN` unset its own per-pid database, and its docstring is explicit about why: every fixture that touches Postgres truncates the graph tables, so two runs against one database delete each other's rows. `test_binding_status.py` -- written earlier the same day -- inlined the development DSN and so opted itself out of that protection. **Every run of it truncated the database the console reads.** Found while standing a beta up: the seed vanished twice, and both times it looked like data that had never landed rather than data something had deleted, which is the expensive half -- a hardcoded DSN does not fail, it removes somebody else's rows and leaves a screen reading as a feature that does not work. **Prose could not have prevented this**: `conftest.py` explains the scheme at length, at the top of the file every test module sits beside, and it was read and not applied. So the fix is the check -- `test_no_hardcoded_dsn.py` refuses a module-level constant bound straight to the development database, proven red by reverting the fix and watching it name the file. **Narrowed after its first version was wrong**: matching the literal anywhere flagged three modules that never connect -- one sets an environment variable for a child, one passes it to a function taking a fake store, one is the lint's own planted fixture -- and a lint that fires on those is one somebody widens `ALLOWED` on until it covers nothing | landed | `tests/test_binding_status.py`, `tests/test_no_hardcoded_dsn.py` |
| CI-W544 | **The installer's own check told two lies about the machine it was checking.** Owner is about to test the npx path, and both would have landed in the first minute. **`--check` reported every previous run as a crash**: `runCheck` passed `alive: false` unconditionally to `previousRunVerdict`, so a reader with a perfectly healthy Postgres was told the process "is gone, so it did not shut down cleanly" and a fresh one would be started -- alarming, wrong, and the opposite of what `--no-admin` does with the same record. It now probes with `clusterServing()`, which is `pg_ctl status`: it reads, starts nothing, and is **the same probe the install path already branches on**, so the two cannot disagree about one machine. **And the caveat said the installer did not exist**: it read "the download, the process start and the port bind are not written yet" while those steps sat below it in the same file, where `--no-admin` runs them -- a sentence that outlived the gap it described, printed as the first thing a newcomer reads, telling them not to try the thing that works. The replacement keeps only what is still true, which is narrower and is what Decision 99 actually needs: this command reports rather than installs, and the unproven case is a machine that never had the repository. The guard asserted the retired wording, so it now asserts the new claim **and that the old one cannot come back** -- proven by running the command, not by reading the file. Landed from a scratch worktree at `origin/main`: the shared checkout holds three sessions' uncommitted work and one file of 346 lines mid-edit, so a fast-forward there would have clobbered it | landed | `bin/sync-up.mjs`, `tests/test_container_install.py` |
