...
| CI-W610 | **Eight commits of stranded UI work are recovered and landed.** The owner's other Claude account ran a session named *UI Design V2* against this same repository and branch, hit its weekly usage limit mid-run, and left `worktree-ui-workflow-nav` eight commits ahead in `.claude/worktrees/` -- the live workflow thought-process panel, the Telemetry page as a live traffic instrument, the run activity feed, and the findings Solution column. It branched from `d0085d30`, the same commit this session branched from, so the two lines were parallel rather than competing: that one built visible features, this one built the chassis. Ten conflicts resolved by intent rather than by side -- **both** optional seams kept where each line added a different one (`slice_for` and `activity_for`); the branch's Telemetry rewrite taken whole because the owner ruled for it twice, then wrapped in the `ScreenFrame` it predates; the owner-picked API surface panel (W541) restored where the branch's wip base had dropped it; `RouteEntry.question` left deleted because `CI-W597` removed the field the branch re-populates. The findings page needed no merge at all: its Solution column arrived through `findings-table.tsx`, which merged clean. Gates: tsc 0, **vitest 126 files / 1,059 tests** (up from 1,045 -- the branch brought its own), lint 0, build 0, pytest 4,549 | landed | 10 conflicted files, `web/`, `src/sync/`, `tests/` |
| CI-W612 | **Each sidebar area says what it does, and the reason the console 'looked the same' is found.** The owner's mock puts a sentence under every sidebar area; the rail carried five bare stage words. `STAGE_DOES` already held the owner's own wording for all five stages and nothing rendered it, so the rail now does -- hidden rather than removed at rail width, where there is no room for a sentence and the icons must not move. **The larger finding is why three sessions of console work read as unchanged: the sidebar persists a `minimised` state in `localStorage` (`sync-console-sidebar-pinned`), and in that state every label is `sr-only`.** The owner has been looking at a 48px icon rail, so no amount of work on labels, grouping or descriptions could have been visible. Confirmed by screenshot in both states | landed | `web/src/layouts/app-frame.tsx` |
| CI-W613 | **The Claude-app session's work is audited end to end: nothing was lost.** Its 18MB transcript is on this machine -- the app and the CLI share `~/.claude/projects/`, `settings.json` and the repo's `.claude/`, so only conversation context fails to carry between them. Verified: **13 work items on `main`**; **8 commits** landed as `CI-W610`; **5 commits** in the customer repo, which is clean with zero unpushed; **five worktrees** with zero uncommitted files; **no stashes**; **26 dangling commits**, every one superseded, including a `change units, page not wired` wip that **is** wired on `main`; **6 unreachable commits** that are 2026-08-19 duplicates whose two reverted items were both rebuilt; and **3 preserve branches** holding 21 files `main` lacks, every one deliberately deleted later by `CI-W597`, `CI-W598`, `CI-W599` or the Telemetry rewrite. **The first sweep missed the branch because it filtered by name** -- `preserve/`, `backup/`, `wip/` read as archives -- and the correction is `git branch --no-merged` plus `git fsck --lost-found`, which do not care what a branch is called | landed | `docs/superpowers/references/notes/2026-08-24-app-session-recovery-audit.md` |
| CI-W614 | **The Stitch token layer lands: one palette change reskins 245 files.** The owner supplied a new design system (`docs/stitch_sync_developer_console/`, 24 screens) plus a CSS token sheet and technical specification, and ruled: use it for everything. This is pass one of *density first, motion after*. **The token names are unchanged and only their values move**, so every screen reskins without being edited -- surface `#131413`, card `#181a19`, popover `#1b1d1c`, ink `#edefee`, primary emerald `#45cd8e`, and the status ramp good/warning/serious/critical taken verbatim from the owner's sheet. Manrope and JetBrains Mono are linked with system fallbacks. Shell geometry the design names but nothing here had -- sidebar 240px, collapsed 48px, topbar 48px, rows 32/36/40 -- are now tokens, because two screens choosing their own sidebar width is the drift a token exists to stop. **Every text pairing was measured before a value was written**: all eight inks clear the 5.05:1 floor on all four surfaces, lowest 5.79:1; black on emerald is 6.53:1. The one sub-floor pairing found, `outline` on `surface-container-high` at 4.54:1, is a border colour and is recorded as border-only. Radii needed no change -- the console's 6px/8px already matched the spec | landed | `web/src/index.css`, `web/index.html` |
| CI-W615 | **The shell becomes the persistent 240px sidebar the specification asks for.** Pass one, step two. **Most of the shell already conformed and the measurement is the useful part**: the sidebar's two widths are already 240px and 48px, argued in `DESIGN.md` under *chassis widths*, and the top bar is already 48px carrying global search, the environment switch and breadcrumbs. Two real gaps. **The default flipped back to expanded** -- it defaulted to a rail since hover-expand landed, and the Stitch specification says *a persistent 240px sidebar with a collapsed 48px icon-only state*, where persistent is the word that decides it. Hover-expand and the pin both stay; only the unchosen reader's starting state moved, and a reader who collapses it keeps that across reloads. **The current destination now carries the emerald edge** -- a 2px right border and a 10% tint, applied in our layer rather than by re-authoring the vendored Supabase primitive. It is marked three ways over, which the design system asks for in as many words (*colour is never the sole channel*): the edge, `aria-current`, and the label -- and at rail width the edge still lands with every name hidden. Four hover tests assumed the rail default and now collapse the panel explicitly first | landed | `web/src/layouts/sidebar-collapse.ts`, `app-frame.tsx`, two test files |
| CI-W616 | **The 36px row is a floor, because measuring showed the overage was content.** The specification asks for a strict 36px row. Measured on the call-sites table with `getBoundingClientRect`, every row came out at **39px** -- and the first two explanations were both wrong. It is not padding drift and not the border: the leading cell **stacks two 16px line boxes**, a file path over a muted second line, inside the same 8px padding. Clamping to 36px would clip the content the screen exists to show, so the row takes `min-h` -- the rhythm holds where a row has one line and yields where it has two. **Two failed attempts are recorded because each teaches something**: `h-row-md` generated no class at all, since Tailwind v4 derives `h-*` from `--spacing-*` and the token is `--row-md`; and overriding the cell's padding from our side does nothing, because the vendored cell merges its own `py-row` in a separate `cn()` call that tailwind-merge never sees beside ours -- both classes land on the element and CSS order decides | landed | `web/src/components/data-table.tsx` |
| CI-W617 | **The Stitch design enters the repository, because a design nobody else can read is not an authority.** 6.2MB: 24 screens with rendered HTML and stills, a design-system `DESIGN.md`, and the owner's technical specification and CSS token sheet. Tracked for the same reason `docs/console-mock/` is -- the token layer, the shell and every screen after it are measured against these files, and a later session that cannot open them cannot check the work or continue it. This is the second time in one session that unopened design material cost a day: `CI-W607` records the first | landed | `docs/stitch_sync_developer_console/` |
| CI-W618 | **Call sites leads with the call sites.** The Index screen opened with two full chart panels and put the filter rail, the table and the detail pane below them, so a reader who came to read call sites scrolled past `Call sites per integration` and `Most-called operations` to reach one. The design system's first principle is density and its layout rule puts the main data view in the content band; the owner's own Index mock leads with the explorer and carries no dashboard row at all. The dashboards move below and keep the property that mattered about their placement -- still outside the table's success branch, because the topology answers a different question over the same rows and a failure to page the table is no reason to withhold the shape of what was indexed. **Checked across every other screen: call sites was the only one with a chart ahead of its data** | landed | `web/src/features/bindings/call-sites-page.tsx` |
| CI-W619 | **The traffic chart drew failures and successes in two near-identical oranges.** Found by looking at the Observe screen, not by a test. `errored` was pinned to the reserved serious ink and `succeeded` was left to take categorical slot 0 -- which is itself an orange, so the two stacked bands were nearly the same colour and the legend was the only thing telling a reader which was which. **The file already carried a comment about this failure in its inverse form**: a categorical slot once rendered errors green and successes orange, and pinning one member fixed that instance without fixing the class. Both bands are states, so both now take reserved status inks -- serious and good. The guard asserts each member is pinned *and* that the two differ, which is the half that would have caught this: both colours were individually valid. Red-proved by putting `succeeded` back on a categorical slot | landed | `web/src/features/telemetry/traffic-over-time-card.tsx`, `traffic.test.ts` |
| CI-W620 | **Solutions leads with its facts, and the earlier survey that missed it was too narrow.** The Remediate screen opened with a full-width Sankey roughly 600px tall, so the KPI strip -- which this page's own comment says comes before its rows, by the owner's ruling of 2026-08-19 -- was entirely below the fold, and the table below that. The funnel moves under the rows. **`CI-W618` claimed call sites was the only screen with a chart ahead of its data and that claim was wrong**: the check grepped for component names like `Dashboards` and this page's chart is an inline `TicketFunnel`, so the pattern never matched. Re-run over every `*-page.tsx` by position inside `<ScreenFrame>` against a chart/data vocabulary, the answer is now **none**. **Flagged, not fixed:** `Repair attempts over time` paints `abandoned` in the good ink and `retried` in an off-palette pink. It is the traffic chart's bug again -- states taking categorical slots -- but five outcome states exceed the three reserved status inks, so the fix is a five-step outcome ramp argued in `DESIGN.md`, which is the owner's document | landed | `web/src/features/workflows/solutions-page.tsx` |
| CI-W621 | **The outcome ramp lands, and running the Python gate found six commits' worth of red.** Owner ruling: five outcome states get their own ramp. **The ruling's own middle steps did not survive measurement** -- `opened` against `retried` came out at CIE76 **dE 2.1**, two colours no reader can tell apart, which is the exact defect `CI-W619` had fixed one screen earlier; `reported` against `abandoned` was 19.8. The endpoints are the owner's unchanged and the middles were searched for the arrangement maximising the worst pair: **worst is now dE 26.3**, every step clears 5.05:1 on all three surfaces. `tests/test_outcome_ramp.py` holds both floors and carries the paired violation tests. **The gate failure is the finding.** Running `uv run pytest` revealed the ink guard I wrote in `CI-W601` had been red since `CI-W614` -- it asserted `oklch(` and the owner's token sheet is hex -- and stayed red through six commits, because `index.css` changes were gated on the console suite alone while that check lives in the Python one. Also caught: three shell-geometry tokens added in `CI-W614` that `DESIGN.md` argued for nowhere. Both fixed; the skip baseline moves 18 to 19 | landed | `DESIGN.md`, `web/src/index.css`, `echart.tsx`, `remediation-activity.tsx`, three test files |
| CI-W622 | **Every screen names itself, at a size larger than its own sections.** Eval loop iteration 1, comparing Findings against Stitch's `self_healing_queue`. **Measured, not guessed:** the page's `h1` rendered at **13px** while every `h2` inside it rendered at 18px -- a page titled smaller than its subsections. The heading lived in the breadcrumb trail, where body size is right for a trail segment and wrong for a document's top-level heading. It moves to the top of the screen at `text-page` (22px), above the controls, which is where every Stitch screen draws it and the order the four bands describe. **Still read from the route registry**, so the nav, the trail and the heading cannot disagree -- that half of the original argument was right and is kept; the trail keeps the same words as a segment marked `aria-current`. **Two prior rulings are superseded and say so in their tests**: *carries no page header, because density is dense* on two screens, and *the trail names the page so a page does not have to*. A high-density design is not one without titles -- the Stitch screens are denser than ours and every one is titled. There is still exactly one `h1`; it moved rather than multiplied | landed | `screen-frame.tsx`, `scope-switchers.tsx`, four test files |
| CI-W624 | **The shell sheds its prose: furniture stage labels, bare items, a chip for the environment.** Owner ruling 2026-08-25 -- too much text in the sidebar and top bar, sizing and fonts off the Stitch register. The five `STAGE_DOES` sentences come back out of the rail (reversing `CI-W612`'s rendering, not its vocabulary -- each sentence survives as its heading's `title`, one hover away); stage headings move to the furniture register (uppercase, tracked, `ink-secondary`); nav rows go h-7 to h-9, the 36px rhythm. The top bar's `local dev · git: <login>` sentence becomes a bordered `LOCAL DEV` furniture chip with the forge login in its tooltip -- stated fully on Getting-started, which is its home. **The parity eval's synthesis independently ranked both of these among its 12 systemic findings** (#7 sidebar prose, #12 topbar identity), from captures taken before the fix | landed | `web/src/layouts/app-frame.tsx` |
| CI-W627 | **Three systemic fixes land, and the layout rulings are taken screen by screen.** Eval findings #10, #11, #4: the chrome lifts onto raised surfaces (sidebar `#1b1c1b`, topbar on `secondary`, over the `#131413` page -- ink arithmetic noted at the token); the face splits (Inter leads `--font-sans` for body and meta, Manrope becomes `--font-display` on h1-h3, Inter joins the font link); the active nav item becomes the filled primary pill with `primary-foreground` ink at 6.53:1. Finding #3 (tinted badges) turned out **already built** -- `Tag` ships icon+word on tinted status surfaces, and rung is neutral by the documented ruling that colouring evidence would restate the rejected confidence scalar -- so it becomes per-cell application work in the screen passes, not a component change. **The owner ruled every layout**: full rebuilds for Overview (bento), the three list screens (locked+inspector), the two investigation screens (evidence/remediation split), Solutions (board), Graph (canvas+inspector); tables stay for Vendors/Services/Telemetry; the tail reskins. Order: frame mechanics once in ScreenFrame, then by traffic | landed | `web/src/index.css`, `app-frame.tsx`, `web/index.html`, the plan |
| CI-W628 | **The cognitive-load pass: the owner's seven removals, and Getting Started becomes a stepper.** Owner rulings 2026-08-25, item by item. Gone: the Scope sentence (the trail and switcher already scope the page), the pipeline's not-to-scale paragraph, the API surface panel (component deleted, not flagged off), the settings-door descriptions (each survives as its row's tooltip), and the top bar's large mono repo identity (demoted to meta, muted). **Getting Started leads the Overview and is rebuilt on the owner's Superlog example**: icon bubble, N-of-M READY with a progress bar, one icon chip per prerequisite, and the first unmet one expanded with its fix line and the single CTA -- the ready-count supersedes the card's old no-figure note by ruling, and unanswered stays distinct from missing because a probe that never ran must not claim an answer. **Vendors is cards only** -- the table view retired, not toggled off. `--font-display` gets its DESIGN.md row, which `CI-W627`'s gate missed because output buffering hid the second FAILED line -- the same lesson as `CI-W621`, now twice | landed | overview, vendors, getting-started, workflow-grid, app-frame, DESIGN.md |
| CI-W629 | **The chrome becomes two bars, the sidebar moves only on its button, and the trail is parent/current.** Owner rulings 2026-08-25, several in one pass. **The stats bar**: every screen's KPI strip and the pipeline now portal into a second 48px chrome row, full width, each cell an evenly-divided instrument with its label centred over its value -- one mechanism in `topbar-stats.tsx`, so all fourteen KPI pages moved without being edited, and `empty:hidden` keeps the row off a screen that publishes nothing. **The sidebar**: hover-expand is deleted and the button is the whole mechanism -- a pointer crossing the rail no longer reflows the page -- and at rail width the wordmark is *absent* rather than `sr-only`, because an invisible element still took width and pushed the toggle off centre, which was the overlap reported. **The trail**: parent / current, the repository by its tail name with the full id one hover away, the vendor level only inside a vendor scope, and the page tabs moved out of the chrome. **Vendors**: the All/Generated tabs went (every attached vendor is generated, so the control offered one answer twice) -- and going cards-only had silently dropped an honesty distinction the table carried, so `VendorCard` now takes `catalogueAnswered` and says *the catalogue did not answer* rather than printing a tier it never read | landed | `topbar-stats.tsx`, `app-frame.tsx`, `scope-switchers.tsx`, `kpi-strip.tsx`, `pipeline-strip.tsx`, `vendor-card.tsx`, six test files |
| CI-W634 | **The shared chrome layer: four primitives four specs each invented separately.** Step two of the rebuild sequence, and the one step with no visible payoff -- which is why it stays small. `layouts/pane.tsx` holds the mechanic (`Pane`/`PaneScroll`): four classes whose failure mode is silent, since a flex child without `min-h-0` refuses to shrink and the page grows a second scrollbar nobody meant. `components/pane.tsx` adds the banded chrome the references draw, settling four specs' disagreements about the header strip once -- 40px, `secondary` over `card`, furniture label -- with a `footer` slot because a count inside the scroll scrolls away from the rows it counts. **`DetailLayout` gains `docked`**, which renders its aside with nothing selected (a resting pane beats a table that reflows on every click) and drops the `calc(100svh-8rem)` cap that was measured against a one-bar chassis -- and the undocked default is byte-identical, proven by its seven existing tests passing untouched. `TableHeader` gains `sticky` (opaque, not the alpha step -- an alpha composites against the rows scrolling under it) and `TableFrame` gains `fill`. `ScreenFrame`'s `fill` boolean becomes `layout="flow"|"fill"|"locked"`. Guard red-proved by deleting one link of the shrink chain | landed | `layouts/pane.tsx`, `components/pane.tsx`, `detail-layout.tsx`, `data-table.tsx`, `screen-frame.tsx`, `pane.test.tsx` |
| CI-W635 | **Call sites becomes a locked explorer, the detail becomes a drawer, and the console goes full width.** Step three, with two owner corrections landed the same session. The screen stamps  -- the first to do so -- which closed the positive half of the frame's  behaviour: measured live,  flips to , the document does not scroll, and only the panes carry scrollbars. **Two structural mistakes of mine, both caught by the owner's captures**: I nested a grid inside , which already owns the table/inspector split, and the table was crushed until file paths wrapped one word per line; and the docked column was the wrong shape entirely -- the detail is a right-hand **drawer over** the page now, so a reader working a table keeps the table at full width.  is deleted on every route: 1920 is the width this console is operated at, and the cap left 520px of dead page beside a table that wanted it | landed | , , , ,  |
| CI-W636 | **Nothing is clipped, on either kind of screen, and chrome scales with the viewport.** Two bugs, opposite directions, both from the chassis lock and both found by the owner scrolling rather than by a test. A **flowing** screen carried `min-h-0` on its content box, which clamped it to the viewport: 986px of content held at 869px, 117px of scroll, the rest unreachable. A **locked** screen had the inverse -- its table was bounded by nothing, grew to 1601px inside a 925px region, and inflated the screen it was supposed to be bounded by. One rule with two directions: a flowing box must be free to grow, a bounded box must actually bound, and neither may clip. `data-screen` now stamps `fill` as well as `locked`, so the frame can tell them apart through `:has()`. **And a correction to my own first fix**: it hid the chassis qualifications at short viewports with `display: none` -- one of those is the multi-tenancy claim that nothing is filtered per viewer, and `console-surface.md` forbids collapsing a protected sentence behind any disclosure, a media query included. Only spacing scales now | landed | `app-frame.tsx`, `screen-frame.tsx`, `detail-layout.tsx`, `index.css`, `pane.test.tsx` |
| CI-W638 | **The shell takes a real height chain, and the scrollbars stop drawing light.** Owner report: the console was shorter than the window on another monitor, leaving a black band beneath it, and "still white bars". **`html`, `body` and `#root` carried no height at all.** The locked chassis asked for `100svh`, which is viewport-relative and so appeared correct -- measured at 1080, 1131, 864, 1440 and 768, it filled exactly, gap 0 every time. What that measurement could not see is display scaling: under Windows scaling or browser zoom the visual and layout viewports separate, `svh` follows one of them, and nothing underneath corrects the shell. A real chain has no such failure mode -- each element is 100% of a parent that is 100% of the window, one source of truth rather than a unit that resolves per device. **The scrollbars were never styled at all**, so every scroll region drew the platform's light scrollbar over a dark console; both syntaxes are declared, since Firefox reads `scrollbar-color` and Chromium the pseudo-elements, and no token is new -- the thumb is the `input` step a control's border already takes | landed | `web/src/index.css`, `web/src/layouts/app-frame.tsx` |
| CI-W639 | **Solutions becomes the reference's board, built by a workflow agent in an isolated worktree.** One of four parallel builders survived; three hit the account's session limit mid-run and returned nothing, which is worth recording as the cost of fanning out at that width. What landed: a 2/3 + 1/3 grid of hairline cards with tinted header strips, via one additive `frame?: "plain" | "board"` prop on `MetricPanel` whose default leaves every other panel untouched. **The agent found a real honesty defect I had not**: `SolutionsFunnelRegion` returned `null` for pending, errored and empty alike -- three different nothings rendered as one blank -- and now says which. It also removed a KPI that restated the table's own count (the duplication `metric-panel.tsx`'s docstring already records being removed once), replaced a percentage whose denominator lived only in a tooltip with `{n} of {total}`, and corrected a caption claiming to be "the only one showing the gap between them" while the funnel on the same page showed it too. Its eight new tests were red-proved by the agent against deliberately broken subjects | landed | `solutions-page.tsx`, `metric-panel.tsx`, `remediation-flow.tsx`, `remediation-activity.tsx`, `ticket-funnel.tsx`, one new test file |
| CI-W640 | **The limits on UI creativity are retired, the living-motion tier is authorized, and a synthetic corpus exists to develop against.** Owner ruling: remove everything constraining UI capability. `interface-originality.md`'s opening prohibition -- *never for how a screen should look* -- is **retired**; it was written to stop a clone and produced the flatness `2026-08-06-why-the-console-came-out-flat.md` measured. Visual reference is now unrestricted. `DESIGN.md`'s Motion section gains the **living tier**: shaders, Three.js, staged entrances, animated throughput, gated only on not claiming a time the data lacks, honouring `prefers-reduced-motion`, and running on the GPU. **Four refusals stay and the brief says why each is not an aesthetic limit**: no composite score (the product's own argument, and a badge from a closed vocabulary looks identical to the reference's chips), absence-is-not-zero, real data only, and the 5.05:1 contrast floor. `scripts/seed_synthetic.py` writes a corpus wide rather than realistic -- every rung, every severity, every finding status, telemetry present and absent, an error window with no calls behind it, a unicode path, a path long enough to wrap, a loop, an almost-empty repository. **It refuses to manufacture one state**: `insert_finding` reserves `unattributed` for rows written before the column existed, so a corpus that wrote one would assert a history this database does not have | landed | `.claude/rules/interface-originality.md`, `DESIGN.md`, `scripts/seed_synthetic.py`, the master brief |
| CI-W641 | **The synthetic corpus spans all twelve protocol kinds, and the Vendors page gains an honest Add-vendor drawer.** Two workflow agents in isolated worktrees. The corpus now carries 30 vendors across rest-sdk, rest-openapi, llm, mcp, graphql, grpc, webhooks, streaming, queues, soap, auth and baas -- each written in the wire shape its kind actually has, so a webhook vendor's call sites are handlers reading event fields and a queue vendor's are producers and consumers, not generic rows. 60 call sites, 216 vendor changes over four `ChangeSource` values, 175 findings over twelve detectors, all four rungs, all three finding statuses, and all three binding statuses drawable for the first time. **The agent found a bug in the script I wrote yesterday**: `remove()` called `GraphStore.forget_repository`, which does not exist, and its fallback deleted from `finding` by `repo_id`, a column that table does not have -- so `--remove` raised and the corpus could only ever grow. Findings now delete through their call site and changes through a `raw->>'synthetic'` stamp. Two idempotence defects fixed with it. The Add-vendor drawer is honest about a read-only API: it lists registered-but-unwatched vendors, says a vendor appears once a call site to it is indexed, and for an unregistered one shows the exact `generated-vendors.yaml` block rather than claiming the console performed an addition | landed | `scripts/seed_synthetic.py`, `add-vendor-drawer.tsx`, `repository-vendors-page.tsx` |
| CI-W642 | **The file that loads on every console edit pointed at the retired mock, which is why the reskin kept happening.** A five-agent audit over all 46 plans and every spec confirmed the owner's report and found the mechanism. `web/CLAUDE.md` opened with *"Before you change a screen, open the mock -- `docs/console-mock/screens/`"* and routed to the mock-parity plan. That mock was demoted on 2026-08-25 when the Stitch set became the visual authority, but **both directories sit side by side on disk**, and an agent obeying the every-turn rule opened the wrong drawing and conformed to the old UI. The paragraph now names `docs/stitch_sync_developer_console/`, says plainly that `console-mock` is retired, routes to the master brief and the executable specs, and carries the measurement that makes the distinction concrete: **eighteen of twenty-one screens still render `ScreenFrame` at its default `flow` layout** while six are mandated as full rebuilds into locked multi-pane compositions. A token swap reskins every screen without editing one, which is exactly how a console changes palette and stays the old console. The brief's own miscount -- "five remaining rebuilds" over a list of six -- is corrected | landed | `web/CLAUDE.md`, the master brief |
| CI-W643 | **The audit's findings and the tuned dispatch prompts survive the session.** The reconciliation that proved the console was reskinned rather than rebuilt goes into the master brief as section 6a, with the measurement that makes it checkable -- eighteen of twenty-one screens on the default `flow` layout, and the files marked DELETE that still exist as the quickest test of whether a rebuild happened. Two workflow scripts move out of the session directory into `docs/superpowers/orchestration/`, because both are repeatable and both took real effort to get right: the screen-rebuild contract, tuned against the failure it exists to prevent, and the plan reconciler, which is told not to trust checkboxes because `CI-W607` found a plan reading 0-of-74 while 89 items had shipped. Two memories added -- the rebuild debt, and the lesson that an always-loaded pointer outlives the material it names | landed | the master brief, `docs/superpowers/orchestration/` |
| CI-W644 | **Vendor marks can be bundled, and the directory ships empty on purpose.** Owner ruling 2026-08-25, amending the monogram-only rule of 2026-08-19. `VendorMark` now resolves `assets/vendors/<vendor_id>.svg` through a build-time `import.meta.glob` and falls back to the generated monogram when there is no file -- so a mark that is present is used, and the console never has a hole where one should be. **Two of the three objections that removed logos were about the fetch, not the logo**: Clearbit learned which integrations each customer watched, and it made the console's appearance depend on a network nobody controls. A committed file does neither. **The third stands, and is why the directory is empty**: a vendor's logo is their trademark, and which marks Sync may ship is the owner's call per vendor rather than something to fill in speculatively. The existing guard asserted *no `img` at all*, which was right while the monogram was the only mark and would now fail for a reason unrelated to what it protects; it asserts the **origin** instead -- no `src` may be remote, whichever mark is drawn | landed | `vendor-mark.tsx`, `vendor-mark.test.tsx`, `web/src/assets/vendors/README.md` |
| CI-W645 | **Wave 1 of the Stitch rebuild: Findings, Overview and Runs are rebuilt, not reskinned.** Three isolated worktrees, each given its screen's KEEP/REBUILD/DELETE list from `2026-08-25-stitch-rebuild-specs.md` and the matching Stitch still. All three land on a bounded layout -- **seven screens now render `locked` or `fill` against three before**, which is the measurable answer to *"everything looks exactly the same"*: a token swap cannot produce it. Deleted rather than restyled: `TriageTabs`, the change-unit expander, `GroupingToggle`, the `MetricPanel` wrapper, `DismissedTally` on Findings, and `fleet/runs-table.tsx` whole. `AbandonReason` and the four run judgements moved to `features/runs/`, where they are testable without mounting a table. Findings verified in Chrome at 1920x1080: `main` at `overflow: hidden`, `scrollHeight == clientHeight`, and a 640px drawer that never squeezes the 1583px table -- the owner's *"the call site should be a drawer view and not compact the screen"*. Suite 128 files/1079 tests to **135/1136** | landed | `findings-page.tsx`, `overview` bento, `runs-page.tsx`, `run-row.ts`, `abandon-reason.tsx`, `finding-selection.ts` |
| CI-W646 | **Wave 2: Finding detail, Workflow and Graph are rebuilt.** Finding detail and Workflow both land the owner's **evidence/remediation split** -- two panes, each scrolling its own body, measured at 1366x768 as a true 50/50 (515px at x=280 and x=811) with the page itself not scrolling. Graph lands **canvas + inspector**: a 118-node map taking 69% of the content width with the inspector beside it. **Nine screens now render `locked` or `fill`, against three at the session's start.** Workflow's empty state is the honesty rule working -- *"the API answered, and the checkpointer holds no run under this identifier... this is an answer about the run, not a failure of the console"*. Deleted: `run-fact-rail.tsx`, `settled-output.tsx`, and finding detail's whole `DetailGrid` composition with its 360px fact rail and four `MetricPanel` wrappers. Two agents independently factored the same diff renderer out of `patch-panel.tsx` into differently-named modules; `patch-parts.tsx` survived as the superset and `patch-diff.tsx` was dropped at the merge. Suite 135 files/1136 tests to **139/1189** | landed | `finding-page.tsx`, `workflow-page.tsx`, `index-graph-page.tsx`, `evidence-pane.tsx` x2, `remediation-pane.tsx` x2, `graph-inspector.tsx`, `split-panes.tsx` |
| CI-W647 | **The viewport scaling steps were inert for their whole life, and cascade layers are why.** `index.css` put three `max-height` rules in `@layer components`, where they lose to `p-frame`, `py-section` and `gap-8` -- **Tailwind's `utilities` layer outranks `components` regardless of selector specificity**, so a more specific attribute selector still lost. Measured in Chrome at 1366x768 before the fix: the frame gutter rendered **40px where the rule says 8px**, the gap **32px where it says 16px**, the chassis qualifications **16px where they say 8px**. Un-layered, all three hit their values exactly and reclaim ~64px of vertical space at laptop heights -- which is the owner's *"the bottom of most pages are cut off"* and *"scaling is still slightly off"* in their remaining form. Found by a rebuild agent reporting it as out-of-lane rather than silently working around it | landed | `web/src/index.css` |
| CI-W648 | **Vendors is fully cards and Solutions is a board -- both owner rulings, both measured undone the same morning.** Vendors was a *hybrid*: it already carried a `VendorCard` and three `grid-cols` while still holding **eighteen table references**, which is how it read as done and wasn't. Now **zero `<table>` elements render** and a test asserts it; thirty cards, each with its `VendorMark` (bundled SVG, monogram fallback, nothing fetched), its call-site count with the denominator on the same line and a `RungBadge` qualifying it. Solutions was still a table; it is now a board of five columns from the closed disposition vocabulary -- In flight / Parked / Reported / Abandoned / Opened -- and a column with no members still renders its heading and its zero, because an absent column would claim the stage does not exist. Trends and Corpus rebuilt to their references. The charts lane **refused its own reference**: `advanced_telemetry_trace_explorer` carries a pulsing "Tailing..." dot, and a liveness pulse is refused by `web/CLAUDE.md`. It also declined to add ambient motion despite the brief authorising it, because `MOTION_USAGES` is a closed registry asserted in both directions by `tests/test_console_design_tokens.py` -- motion needs a registry entry argued first. Suite 139 files/1189 tests to **144/1250** | landed | `repository-vendors-page.tsx`, `vendor-card.tsx`, `vendor-deck.ts`, `vendor-inspector.tsx`, `solutions-board.tsx`, `metrics-page.tsx`, `precedent-page.tsx` |
| CI-W649 | **`.bento-lock-3`, because an implicit grid row is sized before the `fr` rows divide what is left.** Trends took `RemediationFlow` and `AttemptsByTier` when Solutions became a board -- two lanes each edited `metrics-page.tsx` and only one of them owned it. Dropping the panels into `.bento-lock`, which declares **two** tracks and `overflow: hidden`, put them in an implicit third row: measured at 1920x1080 the two chart rows collapsed to **131px and 87px** while the new row took **399px**, and two panes clipped inside their own boxes. The new variant declares three tracks and scrolls rather than locking, so seven panes are not squeezed into one screen. Its third floor is **27rem** because `AttemptsByTier` measures 383px at 1920 and **419px at 1366**, where the narrower column makes its rows taller -- a floor under the taller number clips it at the resolution the brief names as the one that must also hold. `RemediationFlow` had to arrive rather than merely move: Solutions renders a link reading *"Open findings nothing has attempted, on Trends"*, and a link to a panel that is not on the page is a promise broken on screen. `ticket-funnel.tsx` deleted -- mounted nowhere, no test, its stages are the board's | landed | `web/src/index.css`, `metrics-page.tsx`, `ticket-funnel.tsx` |
| CI-W650 | **The binding surface is four panes, not two screens of scroll.** Owner report: *"unorganized and has cluttered information."* Measured before: **1397px of content in a 720px viewport -- 1.9 screens -- to show FOUR data rows**, through a NINE-column table and 346 words. Now `layout="locked"` over four named regions -- This operation (240px) / Call sites (368px) / Vendor changes (368px) / Selected call site (320px) -- with **five columns, 190 words, and the page itself no longer scrolling**. The lane chose `repository_index_explorer` and argued it: that reference's centre pane is literally titled *"watched call sites"*, and its source-preview pane is backed by payload we already hold (`snippet`, `snippet_start_line`, `source_served`) rather than a form we would have to fake. It also **eliminated `high_density_technical_console` on inspection** -- that directory holds only a `DESIGN.md` token sheet, no screen and no code, so it cannot supply a composition at all | landed | `binding-surface-page.tsx`, `call-site-columns.ts`, `filter-rail.tsx` |
| CI-W651 | **Overview is the landing screen, so the fold is a design decision.** Owner ruling: Getting Started first, the two maps beside each other, the stage doors below. `.bento-lock-overview` sizes row 2 as `calc(100% - 15rem)` -- row 1's floor plus the row gap -- so it resolves to exactly the rest of the FIRST screenful and the doors begin at its bottom edge. A fixed floor cannot do that: 22rem was tried and measured 352px against a 465px grid, which put **the maps themselves** below the fold. Three floors, each set by a measurement rather than a guess: row 1 is 13rem because a bare `auto` gave 72px with the body crushed to 32px, and 9rem then left a 60px shortfall that grew the pane its own scrollbar (owner: *"still need more space for the getting started info"*); row 3 is 31rem because `min-height: 0` collapses a pane's intrinsic contribution, so an `auto` track measured 176px against a body needing 454px. Final: 208 / 224.5 / 496, **zero inner scrollbars on any pane** | landed | `web/src/index.css`, `codebase-page.tsx` |
| CI-W652 | **Telemetry rendered an absence it had not verified, and the seeder made the impossible state.** The screen printed *"telemetry was never attached ... there are no observed calls"* over a payload holding sixty observed calls with traces and error counts. Not a product defect: real telemetry arrives through `sync.telemetry.ingest`, which marks attachment itself (`ingest.py:117`). `seed_synthetic.py` wrote `record_observed_call` straight to the store and skipped that, producing a state the product cannot otherwise reach, and `signals-page.tsx:71` correctly refused the whole screen on the flag. The seeder now calls `mark_telemetry_attached`; Telemetry renders 7 sections, 4 tables, 22 rows and 3 charts | landed | `scripts/seed_synthetic.py` |
| CI-W653 | **Two audit findings closed, both of them things previously reported as done.** (a) `pr_url` was on the payload and rendered NOWHERE on finding detail or the pull-request screen -- an opened pull request read exactly like a patch that never opened one. The spec attached that row to the `patch-parts.tsx` extraction; two lanes factored the same renderer at once, the coordinator kept one, and **the addition went with the copy that was dropped**. Now three states are held apart: opened with a number, opened without one, never opened. (b) **The Settings descriptions the owner asked removed were never touched** -- a commit removed the settings-*door* descriptions on Overview and the plan then reinterpreted the instruction as that. `SettingCard` carried a required multi-paragraph `description` at `text-body` on twelve cards, so the screen read as documentation with controls in the margin. Moved behind the card's info hint rather than deleted, under `console-surface.md`'s rule that the distinction is protected and the paragraph explaining it is not; `refusalNotice` is the slot for claims that must stay visible and still renders in place. The hint label deliberately does not open with "About" -- three tests caught that `getByRole("button", {name: /^About/i})` matched twelve hints as well as the group nav | landed | `patch-parts.tsx`, `setting-card.tsx` |
| CI-W654 | **Wave 4: Detectors, Pull request, Vendor detail and Integration changes.** With these, **eighteen screens render `locked` or `fill` against three when the rebuild began**, and the only three left on `flow` are the three the owner ruled stay that way -- Settings, Telemetry and Services. Suite 145 files/1266 tests to **149/1320**. Merged from four isolated worktrees after the lanes were stopped mid-verify at the owner's instruction; all four typechecked clean in their own worktrees before the merge and the merged tree typechecked clean on the first attempt. Deleted rather than restyled: `detector-accountability.tsx`, `rung-mix-card.tsx`, `patch-panel.tsx`, and five vendor-detail cards (`vendor-change-chart`, `vendor-changes-table`, `vendor-exposure-card`, `vendor-findings-table`, `vendor-sources-card`). The pull-request lane did **not** touch `patch-parts.tsx`, so `CI-W653`'s pull-request row survived the merge intact | landed | `detectors-page.tsx`, `pull-request-page.tsx`, `vendor-page.tsx`, `integration-changes-page.tsx` |
| CI-W655 | **`LIKE` reads `_` as a wildcard, and a real corpus path contains three of them.** The call-site path-prefix filter built `LIKE 'tools/ops/airtable_sync.py%'`, where each underscore matches any character -- so a prefix filter silently matched paths it was never given. `_open_findings_predicate` had already taken `starts_with` for this reason; this reader was the one that had not. **The defect was unreachable until now**: nothing on screen sent a path prefix until the project tree did. Also adds the `by_path` facet, one key per file holding a live call site, so the explorer draws its tree from the corpus rather than from the current page -- a tree built from the page redraws itself every time the pager moves, and a directory appears to lose its files. Found and fixed by another session; verified here at 140 graph-store tests and 4562 across the suite | landed | `src/sync/graph/store.py`, `tests/test_graph_store.py` |
