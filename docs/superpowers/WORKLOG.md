...
| CI-W417 | Decision 45 as far as it can honestly go without a write surface: the dismissal grain declared in schema.sql, its closed vocabulary, and the store reads -- the route stops at the read-only guarantee | claimed | `src/sync/graph/schema.sql`, `src/sync/graph/store.py` |
