...
| CI-W434 | The bus tested where it lies rather than where it works -- quiet, dropped, behind, concurrent -- the banner count labelled for what it actually counts, and the burst cost measured rather than assumed | claimed | `tests/test_graph_views.py`, `web/src/features/index-graph/` |
