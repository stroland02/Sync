...
| CI-W447 | `dev_up` starts the API on `SYNC_API_PORT` and checks readiness on a hardcoded 8787, so taking its own advice makes it answer from another lane server -- the exact defect its docstring exists to prevent | claimed | `scripts/dev_up.py` |
