...
| CI-W524 | **The vendor page leads with what the vendor changed, and the exposure table drops its constant.** Owner re-ruling on the running page, superseding decision 29: until telemetry attaches, the exposure table's rung and traffic columns are constants, so the page opened on its emptiest answer while the changes feed -- the thing that moves -- sat below the fold. The order test was flipped first and watched fail against the old order. **The Repositories column is gone from the exposure card**: this card always asks with a `repoId`, so `repository_count` is 1 on every row by construction, and a constant is not a column -- the unscoped payload keeps the field. **The fact rail stops stating one fact twice**: `Repository scope` and `Findings counted over` carried the same value under different labels. The header prose now reads in the page's own order | landed | `web/src/features/vendors/vendor-page.tsx`, `vendor-exposure-card.tsx`, `vendor-page.test.tsx` |
| CI-W525 | **The vendor page names its data, and everything but the call surface moves into tabs.** Owner rulings from the running page, batched: the panel formerly "What this costs you" is **API calls to {vendor}** -- a header names the data, never a judgement about it -- with its two-paragraph caption moved behind the ⓓ (the claims stay visible: the rung per row, the tri-state Traffic cells, and a one-line static-evidence qualifier). The API-calls table is the page's fixed top answer; **Changes / Findings / Sources are tabs beneath it**, held in a query parameter so a shared address opens on the sender's tab, with only the active table mounted. **One word: vendor.** "Integration" is gone from every header -- Open findings per vendor, Severity per vendor, Vendor changes (the route label too) -- and "Errors and incidents" on this page is **Open findings against {vendor}**, the spec keeping its own vocabulary at the level while the screen speaks the reader's. Four new structural tests, including the unknown-panel fallback | landed | `web/src/features/vendors/vendor-page.tsx`, `vendor-exposure-card.tsx`, five sibling cards, `lib/routes.ts` |
