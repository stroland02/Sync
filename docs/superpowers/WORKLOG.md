...
| CI-W425 | The SSE route that forwards the bus -- `GET /api/repositories/{id}/events`, typed heartbeat so silence is distinguishable from a drop, held open with no lifetime cap by owner selection | claimed | `src/sync/api/app.py` |
