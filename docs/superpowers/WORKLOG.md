...
| CI-W430 | The console consumes the stream: a live banner count over a settled graph, freeze-and-name on drop, and the parity guard taught to read EventSource so the receipt can go | landed | `web/src/features/index-graph/`, `tests/test_api_routes.py` |
