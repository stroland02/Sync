...
| 7 | In-component base 4, one dominant value | 2,4,8,**10**,12,16,20,24,32 | 4,8,**10**,16,20,24,32 | 2,4,8,**10**,12,16,20,24,32 | 2,4,8,**10**,12,16,20,24,32 | 2,4,8,**10**,12,16,20,24,32 | 4,8,**10**,16,20,24,32 | 4,8,16,20,24,32 | 2,4,8,16,20,24,32 | 2,4,8,16,20,24,32 | **fail, 6/9** — B100 |
...
| 2 | Two ink levels plus one accent, never three | 2+accent | 2 | 2 | 2 | 2 | 2+accent | 2 | **3** | **3** | **fail, 2/9** — B103 |
...
where the contract names three plus two exceptions. B100.
...
the wrapper at `run-outcome.tsx:28` — on the two screens carrying the densest evidence. B103.
...
push `Rung` off the viewport. That trade is real and it is the other half of B100: the vendor
...
screen reader's heading list opens with the title of a dialog that is not open. B102.
...
ten rows of a ten-thousand-row table. B100 closes both halves — the off-grid value and the claim.
...
to prevent. **B100** is the only one that costs rows — 10px of cell padding turns a declared 36px
...
contract names three. **B101** is three stale numbers and one stale sentence in the one section of
...
next reader trust the wrong figure. **B102** and **B103** are one line each and both are honesty
...
The row height cannot, until B100 lands: a guard banning fractional spacing utilities would be the
...
- **`DESIGN.md` was not edited**, though §3 finds four of its statements wrong. It is the design
  system workstream's file and two agents are in `web/` right now; B101 carries the correction rather
  than this commit.
