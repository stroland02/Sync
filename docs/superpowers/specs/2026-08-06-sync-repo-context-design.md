...
**Branch:** `superlog-reference`, based on `origin/main`, which carries
`sync.dashboard.graph_views` and the whole console line. An earlier draft of this document was
written against a local `main` that was 207 commits behind and claimed `graph_views` was
unmerged; it is not, and nothing in this design is blocked on it.
