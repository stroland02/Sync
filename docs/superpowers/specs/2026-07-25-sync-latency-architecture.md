...
| M3 | The tier cascade and prompt-cache boundaries. The cascade is built — `build_remediator` at `src/sync/cli.py:119` returns the `TieredRemediator` — and the prompt-cache boundaries are not: `src/sync/remediate/agent_patch.py` sets no `cache_control` breakpoint. |
