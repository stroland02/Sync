...
**A patch that needs a new file abandons rather than shipping one.** `git add -u` never stages an untracked path, and the gate now measures the same tree, so a fix requiring a new module fails verification instead of pushing a branch missing it. That verdict is truthful and it is not useful. Widening to `git add -A` is what Task 10 rejected, because it commits whatever else the agent's tool calls left behind — a build directory, a log, a stray install — and separating a new source file the patch needs from that debris is the unsolved part. The patch agent can already stage a file itself, since anything in the index is both verified and committed, but its scope rules still tell it only to run `npx tsc --noEmit` until clean. That instruction now names a different tree from the one the gate measures, so a retry receives diagnostics it cannot reproduce locally, and correcting it means first deciding what the agent is allowed to stage.
...
Measured once, properly, on that one window: 672,286 breaking records, 327,124 after the noise filter. **None is depth-1.** The shallowest is three segments, the median about twenty-five, and the whole population reduces to four underlying schema changes — leaves `description`, `iin`, `issuer` and `stored_credential_usage` — exploded across 7,468 distinct paths under `error/payment_method/card/generated_from/…`.

Both sides now record and compare paths, anchored at the outermost segment. Against that window a call site reading `error` matches 295,848 records where it previously matched none, and the ceiling is the full 327,124; a call site reading only `id`, `status` and `amount` still matches nothing, which is the filter doing its job. Anchoring also closed a wrong-answer case the old rule had: 81,781 records have the leaf `description`, so a call site reading the genuine top-level `charge.description` matched a change twenty levels below it under `error/payment_method/card`. The failure direction was never purely conservative.
...
Two things this does not do. It is not a security control: `git commit --author` sets that field to anything, so anyone who can push to the branch can present as Sync, and what actually stands against that is the lease plus the customer's own branch protection. And it inspects only the tip, so a stranger's commit sitting beneath a later Sync-authored one is invisible to it and would still be replaced. Walking the range to the merge base would close the second; the first is not closable from this side.
