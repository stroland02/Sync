export const meta = {
  name: 'screen-rebuilds-wave-1',
  description: 'Rebuild Findings, Overview and Runs into locked multi-pane compositions',
...
const SCREENS = [
  { slug: 'findings', ref: 'self_healing_queue',  src: 'features/findings/findings-page.tsx',
    ruling: 'FULL REBUILD: viewport-locked table filling the screen, drawer on row selection. The spec says TriageTabs dies and TriageEmpty replaces it, the change-unit expander dies and units open in the inspector, GroupingToggle and the severity strip become one ChipTabs, and DismissedTally moves to Trends leaving a status-band note.' },
  { slug: 'overview', ref: 'developer_control_center', src: 'features/fleet/fleet-page.tsx',
    ruling: 'FULL REBUILD to a bento grid: viewport-locked, panes tiled to fill the screen, each scrolling internally. The KPI row is already in the chrome. NOTE: the spec may name codebase-page.tsx; the Overview route resolves to fleet-page.tsx — read the route registry in web/src/lib/routes.ts and rebuild whichever file the index route actually renders.' },
  { slug: 'runs',     ref: 'infrastructure_logs', src: 'features/runs/runs-page.tsx',
    ruling: 'FULL REBUILD: locked log-stream layout with a docked run-record pane on selection. The spec says features/fleet/runs-table.tsx is DELETED.' },
]
